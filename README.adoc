...
* *MapStruct*: 1.6.3
* *OpenTelemetry API/SDK*: 1.56.0
* *OpenTelemetry Instrumentation*: 2.21.0 (+ alpha)
* *Maven*: 4.0.0-rc-4 or later
* *PostgreSQL*: 18.0
* *Keycloak*: 26.4
* *Grafana*: 12.2.1
* *Loki*: 3.5.8
* *Tempo*: 2.9.0
* *Prometheus*: v3.7.3
* *OpenTelemetry Collector*: 0.138.0
...
java -Dserver.port=8888 -jar spring-loaddev-config-server/target/spring-loaddev-config-server-*.jar

# 2. Start Discovery Server on 8761
java -Dserver.port=8761 -jar spring-loaddev-discovery-server/target/spring-loaddev-discovery-server-*.jar
...
# 5. Start Admin Server (optional) on 9090
java -Dserver.port=9090 -jar spring-loaddev-admin-server/target/spring-loaddev-admin-server-*.jar
----
...
flowchart TB
    ...
    class DC_GW,DC_CONFIG,DC_DISC,DC_ADMIN,DC_LOADS,DC_RIFLES,DC_COMP,DC_MCP appStyle
