...
///
/// This filter intercepts incoming requests, extracts the original Bearer token from the Authorization
/// header, and delegates to [UmaTokenExchangeService] to exchange it for a permission token using
...
/// and services.
///
/// The filter runs with `@Order(0)` to ensure it executes early in the filter chain, before
/// security filters that may need the permission token.
///
/// **Token Exchange Flow:**
///
/// - Extract Bearer token from Authorization header
/// - Delegate to UmaTokenExchangeService for token exchange
/// - Receive permission token with resource permissions
/// - Store permission token in exchange attributes
/// - Continue filter chain with enhanced permissions
///
/// If token exchange fails, the filter logs the error and continues with the original token,
/// allowing the request to proceed without enhanced permissions.
///
...
public class PermissionTokenExchangeFilter implements WebFilter {
    ...
    ///
    /// This filter skips token exchange for actuator endpoints to avoid polluting logs
    /// with health check requests from Kubernetes probes.
    ///
