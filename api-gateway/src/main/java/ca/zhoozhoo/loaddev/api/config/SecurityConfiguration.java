...
///
/// This configuration defines the security policies and components for the reactive web application,
/// including OAuth2 login, JWT-based resource server authentication, bearer token propagation,
/// and specialized WebClient configurations for service-to-service communication.
///
/// **Key Security Features:**
///
/// - **Public Endpoints:** Actuator health checks, Swagger UI, and OpenAPI documentation
/// are accessible without authentication
/// - **Protected Endpoints:** All other endpoints require OAuth2 authentication
/// - **OAuth2 Login:** Supports user authentication through OAuth2 providers (Keycloak)
/// - **Resource Server:** Validates JWT tokens for API access
/// - **Token Propagation:** Automatically forwards bearer tokens to downstream services
/// - **CSRF:** Disabled for stateless API operations
///
/// **WebClient Configurations:**
///
/// - **Standard WebClient:** For load-balanced service discovery calls
/// - **OAuth2 Client WebClient:** For OAuth2 client credentials flow
/// - **Keycloak WebClient:** Dedicated client for Keycloak server communication
///
...
public class SecurityConfiguration {
    ...
    ///
    /// The underlying HttpClient is created using Spring Cloud Gateway's HttpClientFactory,
    /// which provides:
    ///
    /// - Connection pooling with max 50 connections
    /// - 5-second response timeout
    /// - Metrics and observability support via Micrometer
    /// - Base URL configuration for Keycloak server
    ///
    /// **OpenTelemetry Integration:**
    ///
    /// The WebClient is configured with an [ObservationRegistry] that bridges Micrometer
    /// observations to OpenTelemetry. This enables:
    ///
    /// - **Distributed Tracing:** Automatic trace context propagation using W3C Trace Context
    ...
    /// Keycloak server.
    /// - **HTTP Client Metrics:** Request duration, response status codes, connection pool
    /// metrics, and error rates are automatically collected and exported to OpenTelemetry
    /// collectors.
    /// - **Contextual Logging:** Log correlation with trace IDs for troubleshooting failed
    /// token exchange operations.
    ///
    /// The trace context is automatically propagated from incoming gateway requests through
    /// this WebClient to Keycloak, maintaining the distributed trace across the entire request flow.
    ///
