...
///
/// This initializing bean bridges OpenTelemetry distributed tracing with Log4j2 logging,
...
///
/// The OpenTelemetry Log4j2 appender intercepts log events and injects the current trace
/// context into the Mapped Diagnostic Context (MDC). This allows log patterns to include:
/// ```java
...
///
/// - **Distributed Debugging:** Trace a request across multiple microservices by filtering
/// logs by trace_id, seeing the complete request flow in chronological order.
/// - **Root Cause Analysis:** When a trace shows an error, immediately jump to the
/// associated log entries with full stack traces and contextual information.
/// - **Performance Investigation:** Correlate slow traces with application logs to
/// identify exact operations causing latency (database queries, external API calls, etc.).
/// - **Unified Observability:** Combine metrics, traces, and logs in a single view
/// (e.g., Grafana dashboards) for comprehensive system understanding.
///
/// **Spring Bean Lifecycle:**
///
/// This class implements [InitializingBean], ensuring the appender is installed
...
/// that configures Log4j2's global logging system.
///
/// **OpenTelemetry Integration:**
///
/// The appender uses the application's configured [OpenTelemetry] instance (provided
...
/// the application without requiring code changes to logging statements.
///
/// **Example Log Output:**
/// ```java
/// Without correlation:
...
///
/// The OpenTelemetry appender must be declared in Log4j2 configuration (log4j2.xml or log4j2.yaml):
/// ```java
/// <Configuration>
///   <Appenders>
...
public class InstallOpenTelemetryAppender implements InitializingBean {
    ...
    ///
    /// The OpenTelemetry instance is injected by Spring Boot's auto-configuration,
    /// configured via application properties (OTLP endpoints, resource attributes, etc.).
    ///
    ...
    ///
    /// This method is invoked automatically by Spring after all bean properties are set,
    /// during the application context initialization phase. It calls the static installation
    /// method provided by the OpenTelemetry Log4j2 instrumentation library, which:
    ///
    /// - Registers a Log4j2 appender that intercepts all log events
    /// - Extracts trace context from the current OpenTelemetry context
    /// - Injects trace_id and span_id into Log4j2's ThreadContext (MDC)
    /// - Makes trace context available to all downstream appenders and patterns
    ///
    /// **Thread Safety:** The installation is idempotent and thread-safe. If called
