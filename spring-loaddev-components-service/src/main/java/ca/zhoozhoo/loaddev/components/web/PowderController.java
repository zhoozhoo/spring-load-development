...
/**
 * REST controller for managing powder (propellant) components.
 * <p>
 * This controller provides endpoints for CRUD operations on powder data, including
...
public class PowderController {
    ...
    @Operation(summary = "Full-text search powders", security = {
            @SecurityRequirement(name = "Oauth2Security", scopes = "components:view") })
    @ApiResponse(responseCode = "200", description = "Search results", content = {
            @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = Powder.class))) })
    @GetMapping("/search")
    @PreAuthorize("hasAuthority('components:view')")
        public Flux<Powder> searchPowders(@Parameter(hidden = true) @CurrentUser String userId,
                                                                          @Parameter(description = "Full text search query") @RequestParam("query") String query) {
                return powderRepository.searchByOwnerIdAndQuery(userId, query);
        }
